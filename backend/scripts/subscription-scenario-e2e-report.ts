/**
 * Option A subscription scenario report — uses production service code paths only.
 * No repair scripts. Run: dotenv -e ../.env -e .env -- tsx scripts/subscription-scenario-e2e-report.ts
 */
...
async function createVerifiedOptionABusiness(suffix: string): Promise<{
  businessId: string;
  userId: string;
  email: string;
  password: string;
...
const STARTER_CAPS: SubscriptionCapability[] = [
  "employeeQr",
  "locationQr",
  "tableQr",
  "teamManagement",
...
const BUSINESS_ONLY_CAPS: SubscriptionCapability[] = [
  "qrTemplates",
...
function assertCapabilityMatrix(tier: BusinessSubscriptionTier | null, label: string): string | null {
  ...
  for (const cap of STARTER_CAPS) {
    if (!hasSubscriptionCapability(tier, cap)) return `${label}: missing starter cap ${cap}`;
  }
  const expectBusiness = tier === "premium" || tier === "enterprise";
  for (const cap of BUSINESS_ONLY_CAPS) {
    const has = hasSubscriptionCapability(tier, cap);
    if (expectBusiness && !has) return `${label}: missing business cap ${cap}`;
    if (tier === "basic" && has) return `${label}: starter must not have ${cap}`;
  }
  return null;
}

async function runScenarios(): Promise<void> {
  // 1. New business → Unsubscribed
  try {
    const regEmail = `${tag}-register@caretip-test.local`;
    const reg = await registerBusiness({ email: regEmail, password: "TestPass1!" });
    const businessId = reg.user.businessId;
    const mirror = businessId
      ? await prisma.subscription.findUnique({ where: { businessId } })
      : null;
    const ent = businessId ? await resolveSubscriptionEntitlements(businessId) : null;
    const ok =
      Boolean(businessId) &&
      !mirror &&
      ent?.status === "none" &&
      !ent.hasActiveEntitlements &&
      ent.capabilities.length === 0;
    record(
      "S01",
      "New business → Unsubscribed",
      ok,
      ok
        ? "registerBusiness: no mirror, entitlements none"
        : `mirror=${Boolean(mirror)} status=${ent?.status}`,
    );
  } catch (e) {
    record("S01", "New business → Unsubscribed", false, String(e));
  }

  // 2. Business Trial → Business entitlements
  try {
    const { businessId } = await createVerifiedOptionABusiness("trial");
    ...
    record(
      "S02",
      "Business Trial → Business entitlements",
      ok,
      `plan=${ent.plan} status=${ent.status} caps=${ent.capabilities.length}`,
    );
  } catch (e) {
    record("S02", "Business Trial → Business entitlements", false, String(e));
  }

  // 3–5. Plan purchases
  for (const [id, label, plan, tier] of [
    ["S03", "Starter purchase → Starter entitlements", "basic", "basic"],
    ["S04", "Business purchase → Business entitlements", "premium", "premium"],
    ["S05", "Enterprise activation → Enterprise entitlements", "enterprise", "enterprise"],
  ] as const) {
    try {
      const { businessId } = await createVerifiedOptionABusiness(plan);
  ...
  // 6. Trial expiry → Unsubscribed
  try {
    const { businessId } = await createVerifiedOptionABusiness("trial-expired");
    ...
    const ent = await resolveSubscriptionEntitlements(businessId);
    const ok = !ent.hasActiveEntitlements && ent.status === "none";
    record("S06", "Trial expiry → Unsubscribed", ok, `hasActive=${ent.hasActiveEntitlements}`);
  } catch (e) {
    record("S06", "Trial expiry → Unsubscribed", false, String(e));
  }

  // 7–8. Cancellations
  for (const [id, label, plan] of [
    ["S07", "Starter cancellation → Unsubscribed", "basic"],
    ["S08", "Business cancellation → Unsubscribed", "premium"],
  ] as const) {
    try {
      const { businessId } = await createVerifiedOptionABusiness(`cancel-${plan}`);
      ...
      const ent = await resolveSubscriptionEntitlements(businessId);
      const ok = !ent.hasActiveEntitlements;
      record(id, label, ok, `status=${ent.status}`);
    } catch (e) {
      record(id, label, false, String(e));
    }
  }
  ...
  try {
    const { businessId, email, password } = await createVerifiedOptionABusiness("login");
  ...
  try {
    const { businessId } = await createVerifiedOptionABusiness("webhook-create");
  ...
  try {
    const { businessId } = await createVerifiedOptionABusiness("checkout-sync");
  ...
  try {
    ...
    const ok =
      !basicErr &&
      !premiumErr &&
      basicCaps.length === STARTER_CAPS.length &&
      premiumCaps.length === STARTER_CAPS.length + BUSINESS_ONLY_CAPS.length;
  ...
  try {
    const starter = await createVerifiedOptionABusiness("qr-starter");
    await activateViaProductionPath(starter.businessId, "basic", { status: "active" });
    const business = await createVerifiedOptionABusiness("qr-business");
    ...
    record(
      "S13",
      "QR Studio respects Starter vs Business vs Enterprise permissions",
      ok,
      `starter tableQr=${sEnt.capabilities.includes("tableQr")} business templates=${bEnt.capabilities.includes("qrTemplates")}`,
    );
  } catch (e) {
    record("S13", "QR Studio respects Starter vs Business permissions", false, String(e));
  }

  // 14. Table and location limits
  try {
    const { businessId, userId } = await createVerifiedOptionABusiness("limits-starter");
  ...
  // 15. Branding and Templates inaccessible to Starter
  try {
    const { businessId } = await createVerifiedOptionABusiness("branding-starter");
    await activateViaProductionPath(businessId, "basic", { status: "active" });
    ...
    record("S15", "Branding and Templates remain inaccessible to Starter", ok, `branding=${branding} templates=${templates}`);
  } catch (e) {
    record("S15", "Branding and Templates remain inaccessible to Starter", false, String(e));
  }
...
async function main() {
  ...
  console.log("\n=== Option A Subscription Scenario Report ===\n");
  let passCount = 0;
