...
/** Subtle hospitality texture behind the deep card (Unsplash). */
const CARD_AMBIENT_IMAGE =
...
const INJECTED_STYLES = `
  ...
  .premium-depth-card {
      background: linear-gradient(145deg, #2a1a0d 0%, #0a0a0a 55%, #0A101D 100%);
      box-shadow:
          0 40px 100px -20px rgba(0, 0, 0, 0.9),
          0 20px 40px -20px rgba(0, 0, 0, 0.8),
          inset 0 1px 2px rgba(255, 255, 255, 0.12),
          inset 0 -2px 4px rgba(0, 0, 0, 0.85);
      border: 1px solid color-mix(in srgb, hsl(33 82% 55%) 25%, transparent);
      position: relative;
  }

  .card-sheen {
      position: absolute; inset: 0; border-radius: inherit; pointer-events: none; z-index: 50;
      background: radial-gradient(800px circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(235, 153, 44, 0.12) 0%, transparent 42%);
...
export function CinematicHero({
  brandName = "CareTip",
  tagline1 = "The New Standard for",
  tagline2 = "Gratitude.",
  cardHeading = "Contactless tipping, perfected.",
  cardDescription = (
    <>
  ...
  metricValue = 100,
  metricLabel = "Tips routed",
  ctaHeading = "Start your team today.",
  ctaDescription =
    "Scan. Tip. Smile. The contactless way to appreciate the people who make your day.",
  ...
  useEffect(() => {
    ...
    if (reduced) {
      gsap.set(".text-track", { autoAlpha: 1, y: 0, scale: 1, filter: "none", rotationX: 0 });
      gsap.set(".text-days", { clipPath: "inset(0 0% 0 0)" });
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative box-border flex h-screen w-screen items-center justify-center overflow-hidden bg-background pt-16 font-sans text-foreground antialiased sm:pt-20",
        className
      )}
    ...
    >
      ...
      <div
        ...
        <h1 className="text-track gsap-reveal mb-2 inline-block rounded-2xl bg-primary px-4 py-2 text-5xl font-bold tracking-tight text-primary-foreground shadow-lg md:px-8 md:py-3 md:text-7xl lg:px-10 lg:py-4 lg:text-[6rem]">
          {tagline1}
        </h1>
      ...
      <div className="cta-wrapper gsap-reveal pointer-events-auto absolute z-10 flex w-screen flex-col items-center justify-center px-4 text-center will-change-transform">
        ...
        <p className="text-muted-foreground mx-auto mb-12 max-w-xl text-lg font-light leading-relaxed md:text-xl">{ctaDescription}</p>
        <div className="flex flex-col gap-6 sm:flex-row">
          <Link
            to={primaryCtaTo}
            className="btn-modern-light flex flex-col items-center justify-center gap-0.5 rounded-[1.25rem] px-10 py-4 focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))] focus:ring-offset-2">
            <span className="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Get started</span>
            <span className="text-xl font-bold leading-none tracking-tight">{primaryCtaLabel}</span>
          </Link>
          <Link
            to={secondaryCtaTo}
            className="btn-modern-dark flex flex-col items-center justify-center gap-0.5 rounded-[1.25rem] px-10 py-4 focus:outline-none focus:ring-2 focus:ring-[hsl(var(--primary))] focus:ring-offset-2 focus:ring-offset-background">
            <span className="text-[10px] font-bold uppercase tracking-wider text-neutral-400">Learn more</span>
      ...
      <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center" style={{ perspective: "1500px" }}>
        <div
          ref={mainCardRef}
          className="main-card premium-depth-card gsap-reveal relative flex w-[92vw] items-center justify-center overflow-hidden rounded-[32px] md:h-[85vh] md:w-[85vw] md:rounded-[40px] h-[92vh] pointer-events-auto"
        >
          <div
            className="absolute inset-0 z-0 opacity-25"
            style={{
              backgroundImage: `linear-gradient(180deg, rgba(10,10,10,0.75) 0%, rgba(10,10,10,0.95) 100%), url(${CARD_AMBIENT_IMAGE})`,
          ...
          <div className="relative z-10 flex h-full w-full max-w-7xl flex-col items-center justify-evenly px-4 py-6 lg:grid lg:grid-cols-3 lg:items-center lg:gap-8 lg:px-12 lg:py-0">
            ...
            <div
              ...
              <div className="relative flex h-full w-full scale-[0.65] md:scale-[0.85] lg:scale-100 transform items-center justify-center">
                <div
                  ref={mockupRef}
                  className="iphone-bezel relative flex h-[580px] w-[280px] flex-col will-change-transform rounded-[3rem]"
                  style={{ transformStyle: "preserve-3d" }}>
                    ...
                    <div className="relative flex h-full w-full flex-col px-5 pt-12 pb-8">
                      <div className="phone-widget mb-8 flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="mb-1 text-[10px] font-bold tracking-widest text-neutral-400 uppercase">Today</span>
                          <span className="text-xl font-bold tracking-tight text-white drop-shadow-md">Live tips</span>
                        </div>
                      ...
                      <div className="phone-widget relative mx-auto mb-8 flex h-44 w-44 items-center justify-center drop-shadow-[0_15px_25px_rgba(0,0,0,0.8)]">
                        ...
                        <div className="z-10 flex flex-col items-center text-center">
                          ...
                          <span className="mt-0.5 text-[8px] font-bold tracking-[0.1em] text-[hsl(var(--primary))]/80 uppercase">
                            {metricLabel}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        <div className="phone-widget widget-depth flex items-center rounded-2xl p-3">
                          <div className="mr-3 flex h-10 w-10 items-center justify-center rounded-xl border border-[hsl(var(--primary))]/25 bg-gradient-to-br from-[hsl(var(--primary))]/25 to-[hsl(var(--primary))]/5 shadow-inner">
                            <svg className="h-4 w-4 text-[hsl(var(--primary))] drop-shadow-md" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <div className="floating-badge floating-ui-badge absolute top-6 left-[-15px] z-30 flex items-center gap-3 rounded-xl p-3 lg:top-12 lg:left-[-80px] lg:gap-4 lg:rounded-2xl lg:p-4">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full border border-[hsl(var(--primary))]/35 bg-gradient-to-b from-[hsl(var(--primary))]/25 to-[hsl(var(--primary))]/10 shadow-inner lg:h-10 lg:w-10">
                    <span className="text-base drop-shadow-lg lg:text-xl" aria-hidden="true">
                      ⚡
                    </span>
                  </div>
                  <div>
                    <p className="text-xs font-bold tracking-tight text-white lg:text-sm">Instant payout</p>
                    <p className="text-[10px] font-medium text-[hsl(var(--primary))]/60 lg:text-xs">Funds on the way</p>
                  </div>
                </div>

                <div className="floating-badge floating-ui-badge absolute right-[-15px] bottom-12 z-30 flex items-center gap-3 rounded-xl p-3 lg:bottom-20 lg:right-[-80px] lg:gap-4 lg:rounded-2xl lg:p-4">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full border border-[hsl(var(--primary))]/30 bg-gradient-to-b from-[hsl(var(--primary))]/20 to-[hsl(var(--primary))]/5 shadow-inner lg:h-10 lg:w-10">
                    <span className="text-base drop-shadow-lg lg:text-lg" aria-hidden="true">
                      💬
                    </span>
                  </div>
                  <div>
                    <p className="text-xs font-bold tracking-tight text-white lg:text-sm">Team notified</p>
                    <p className="text-[10px] font-medium text-[hsl(var(--primary))]/60 lg:text-xs">Shared with staff</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="card-left-text gsap-reveal order-3 z-20 flex w-full flex-col justify-center px-4 text-center lg:order-1 lg:max-w-none lg:px-0 lg:text-left">
              ...
              <p className="mx-auto hidden max-w-sm text-sm leading-relaxed font-normal text-white/75 md:block md:text-base lg:mx-0 lg:max-w-none lg:text-lg">
                {cardDescription}
              </p>
            </div>
          </div>
