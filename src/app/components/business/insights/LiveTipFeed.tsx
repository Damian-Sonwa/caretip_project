...
export function LiveTipFeed({ items, loading }: LiveTipFeedProps) {
  ...
  return (
    <DashboardWorkspacePanel title={t("business.tips.live.feedTitle")}>
      {loading && items.length === 0 ? (
        ...
        <p className="px-4 py-10 text-center text-sm text-muted-foreground sm:px-5">
          {t("business.tips.live.feedEmpty")}
        </p>
      ) : (
          ...
          {items.map((tip) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate font-medium text-foreground">{tip.staffName ?? "—"}</p>
