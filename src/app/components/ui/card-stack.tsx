...
export function CardStack<T extends CardStackItem>({
  ...
  // Responsive adjustments based on card width
  const isMobile = cardWidth < 400;
  const isTablet = cardWidth >= 400 && cardWidth < 500;
  
  // Adjust parameters for mobile
  const responsiveOverlap = isMobile ? 0.6 : overlap;
  const responsiveSpreadDeg = isMobile ? 28 : isTablet ? 36 : spreadDeg;
  const responsiveDepthPx = isMobile ? 80 : isTablet ? 100 : depthPx;
  const responsiveTiltXDeg = isMobile ? 8 : isTablet ? 10 : tiltXDeg;
  const responsiveMaxVisible = isMobile ? 3 : maxVisible;
  ...
  const cardSpacing = Math.max(10, Math.round(cardWidth * (1 - responsiveOverlap)));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="pointer-events-none absolute inset-x-0 top-6 mx-auto h-48 w-[70%] rounded-full bg-accent/10 blur-3xl"
          aria-hidden="true"
        />
        <div
        ...
        >
          <AnimatePresence initial={false}>
            {items.map((item, i) => {
              ...
              const rotateX = isActive ? 0 : responsiveTiltXDeg;
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-2xl border-2 border-border overflow-hidden shadow-xl",
...
function DefaultFanCard({ item, active: _active }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full">
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-4 sm:p-6">
        ...
        {item.description ? (
          <div className="mt-2 line-clamp-3 sm:line-clamp-2 text-xs sm:text-sm text-white/90">
            {item.description}
          </div>
        ) : null}
      </div>
